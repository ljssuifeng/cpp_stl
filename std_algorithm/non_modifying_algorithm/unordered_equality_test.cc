...
int main() {
  ...
  if (std::is_permutation(coll1.cbegin(), coll1.cend(), coll2.cbegin())) {
    std::cout << "coll1 and coll2 heve equal elements." << std::endl;
  } else {
    std::cout << "coll1 and coll2 don't have equal elements." << std::endl;
  }

  // checkfor corresponding number of even and odd elements
  if (std::is_permutation(coll1.cbegin(), coll1.cend(), coll3.cbegin(),
                          bothEvenOrOdd)) {
    std::cout << "numbers of even and odd elements math." << std::endl;
  } else {
    std::cout << "numbers of even and odd elements don't math" << std::endl;
  }
}
